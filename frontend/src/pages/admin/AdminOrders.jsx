...
export default function AdminOrders() {
  ...
  return (
    <div>
      ...
      {orders.length === 0 ? (
        <div className="text-center py-16 bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm"><ShoppingBag size={48} className="mx-auto text-gray-300 dark:text-gray-600 mb-3" /><p className="text-gray-500 dark:text-gray-400">لا توجد طلبات</p></div>
      ) : (
                ...
                <div className="mb-2 p-3 bg-blue-50 dark:bg-blue-900/20 rounded-xl text-sm">
                  ...
                  {o.wallet_review_status === 'pending_review' && (
                    <div className="flex gap-2 mt-3">
                      <button onClick={() => handleReviewWallet(o.id, 'approve')} className="flex items-center gap-1 px-3 py-1.5 bg-green-500 hover:bg-green-600 text-white text-xs font-bold rounded-xl transition-all"><Check size={14} /> اعتماد</button>
                      <button onClick={() => handleReviewWallet(o.id, 'reject')} className="flex items-center gap-1 px-3 py-1.5 bg-red-500 hover:bg-red-600 text-white text-xs font-bold rounded-xl transition-all"><X size={14} /> رفض</button>
                    </div>
                  )}
