...
import { FiTrash2, FiMail, FiSend, FiChevronDown, FiChevronUp } from 'react-icons/fi';
...
export default function AdminContacts() {
  ...
  return (
    <div>
      ...
      {contacts.length === 0 ? (
        <div className="text-center py-16 bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm"><FiMail size={48} className="mx-auto text-gray-300 dark:text-gray-600 mb-3" /><p className="text-gray-500 dark:text-gray-400">لا توجد رسائل</p></div>
      ) : (
              ...
              <div className="flex items-start justify-between gap-4">
                ...
                <div className="flex flex-col gap-1 shrink-0">
                  <button onClick={(e) => { e.stopPropagation(); toggleExpand(c.id); }} className="p-1.5 text-gray-400 hover:text-[rgb(0,166,62)] hover:bg-green-50 dark:hover:bg-green-900/30 rounded-xl">
                    {expanded[c.id] ? <FiChevronUp size={14} /> : <FiChevronDown size={14} />}
                  </button>
                  <button onClick={(e) => { e.stopPropagation(); handleDelete(c.id); }} className="p-1.5 text-gray-400 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/30 rounded-xl"><FiTrash2 size={14} /></button>
                </div>
              </div>
              {expanded[c.id] && (
                <div className="mt-4 pt-4 border-t border-gray-100 dark:border-gray-700" onClick={(e) => e.stopPropagation()}>
                  ...
                  <button onClick={() => handleReply(c.id)} disabled={sending[c.id] || !replyText[c.id]?.trim()} className="btn-primary text-sm flex items-center gap-1.5">
                    <FiSend size={14} /> {sending[c.id] ? 'جارٍ الإرسال...' : 'إرسال الرد'}
                  </button>
                </div>
              )}
            </div>
