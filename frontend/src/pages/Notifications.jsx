        ...
        <div className="flex items-center justify-between mb-6">
          ...
          {unread > 0 && (
            <button onClick={handleMarkAllRead} className="flex items-center gap-1 text-sm text-[rgb(0,166,62)] hover:text-[rgb(0,145,55)] font-semibold">
              <Check size={15} /> {t('notifications.mark_all_read')}
            </button>
          )}
        </div>
              ...
              <div key={n.id} className={`bg-white dark:bg-gray-800 rounded-2xl shadow-card border p-4 transition-all ${n.is_read ? 'border-gray-100 dark:border-gray-700' : 'border-[rgb(0,166,62)] bg-green-50/40 dark:bg-green-900/20'}`}>
                <div className="flex items-start justify-between gap-3">
                  ...
                  <div className="flex gap-1 shrink-0">
                    {n.order_id && (
                      <Link to="/orders" className="p-1.5 text-gray-400 hover:text-[rgb(0,166,62)] rounded-xl"><ChevronLeft size={14} /></Link>
                    )}
                    {!n.is_read && (
                      <button onClick={() => handleMarkRead(n.id)} className="p-1.5 text-gray-400 hover:text-[rgb(0,166,62)] hover:bg-green-50 dark:hover:bg-green-900/30 rounded-xl"><Check size={14} /></button>
                    )}
                  </div>
                </div>
              </div>
