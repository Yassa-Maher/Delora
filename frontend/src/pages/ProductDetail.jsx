...
export default function ProductDetail() {
  ...
  if (!product) return <div className="text-center py-20 text-gray-400 dark:text-gray-500 bg-gray-50 dark:bg-gray-900 min-h-screen">{t('product.not_found')}</div>;
          ...
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-card border border-gray-100 dark:border-gray-700 p-6">
            ...
            {product.avg_rating > 0 && (
              <div className="flex items-center gap-1 mb-3">
                {[1, 2, 3, 4, 5].map((s) => (
                  <FiStar key={s} size={18} className={s <= Math.round(product.avg_rating) ? 'text-yellow-400 fill-yellow-400' : 'text-gray-300 dark:text-gray-600'} />
                ))}
            ...
            <div className="mb-4">
              {hasOffer ? (
                <div className="flex items-center gap-3 flex-wrap">
                  ...
                  {hasStockOffer && <span className="text-[10px] bg-orange-100 dark:bg-orange-900/30 text-orange-600 dark:text-orange-400 font-bold px-2 py-0.5 rounded-full">{t('product.offer_until_stock')}</span>}
                  {hasDateOffer && <span className="text-[10px] bg-blue-100 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 font-bold px-2 py-0.5 rounded-full">{t('product.offer_limited')}</span>}
            ...
            <div className="flex items-center gap-4 mb-6">
              <div className="flex items-center border border-gray-200 dark:border-gray-600 rounded-xl">
                {isWeightProduct ? (
                  <>
                    <button onClick={() => setQuantity(Math.max(0.1, +(quantity - 0.1).toFixed(3)))} className="p-2.5 hover:bg-gray-50 dark:hover:bg-gray-700 rounded-r-xl"><FiMinus size={14} /></button>
                    <input type="number" step="0.1" min="0.1" value={quantity} onChange={(e) => setQuantity(Math.max(0.1, parseFloat(e.target.value) || 0.1))} className="w-16 text-center font-bold text-gray-900 dark:text-white bg-transparent border-x border-gray-200 dark:border-gray-600 py-2 [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none" />
                    <button onClick={() => setQuantity(+(quantity + 0.1).toFixed(3))} className="p-2.5 hover:bg-gray-50 dark:hover:bg-gray-700 rounded-l-xl"><FiPlus size={14} /></button>
                  </>
                ) : (
                  <>
                    <button onClick={() => setQuantity(Math.max(1, quantity - 1))} className="p-2.5 hover:bg-gray-50 dark:hover:bg-gray-700 rounded-r-xl"><FiMinus size={14} /></button>
                    <span className="px-4 font-bold text-gray-900 dark:text-white">{quantity}</span>
                    <button onClick={() => setQuantity(quantity + 1)} className="p-2.5 hover:bg-gray-50 dark:hover:bg-gray-700 rounded-l-xl"><FiPlus size={14} /></button>
                  </>
                )}
              </div>
              <span className="text-sm text-gray-500">{isWeightProduct ? product.unit_ar : ''}</span>
              <button onClick={handleAddToCart} disabled={adding} className="btn-primary flex items-center gap-2 flex-1 sm:flex-none">
                <FiShoppingCart size={18} /> {adding ? t('product.adding') : t('product.add_to_cart')}
              </button>
            </div>
              ...
              <div className="flex items-center gap-1 mb-3">
                {[1, 2, 3, 4, 5].map((s) => (
                  <button key={s} type="button" onClick={() => setReviewForm({ ...reviewForm, rating: s })}>
                    <FiStar size={22} className={s <= reviewForm.rating ? 'text-yellow-400 fill-yellow-400' : 'text-gray-300 dark:text-gray-600'} />
                  </button>
                ))}
              </div>
              <textarea className="input-field mb-3" rows={3} value={reviewForm.review_text} onChange={(e) => setReviewForm({ ...reviewForm, review_text: e.target.value })} placeholder={t('product.review_placeholder')} />
              <button type="submit" disabled={submittingReview} className="btn-primary text-sm">{submittingReview ? t('product.submitting') : t('product.submit_review')}</button>
            </form>
          )}
          {reviews.length === 0 ? (
              ...
              {reviews.map((r) => (
                <div key={r.id} className="border-b border-gray-100 dark:border-gray-700 pb-4 last:border-0">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <div className="flex">
                      {[1, 2, 3, 4, 5].map((s) => (
                        <FiStar key={s} size={14} className={s <= r.rating ? 'text-yellow-400 fill-yellow-400' : 'text-gray-300 dark:text-gray-600'} />
                      ))}
                    </div>
                  </div>
