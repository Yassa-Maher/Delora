...
import { Plus, Tag, Check, X, Upload } from 'lucide-react';
...
export default function Checkout() {
  ...
  const { paymentMethods, getSetting } = useSettings();
  ...
  const shipping = parseFloat(getSetting('delivery_fee', lang)) || 30;
  ...
  const handlePlaceOrder = async () => {
    const isWallet = paymentMethod.startsWith('wallet_');
    ...
    if (isWallet && !processNumber.trim()) { toast.error(t('checkout.enter_process_number')); return; }
    if (isWallet && !proofImage) { toast.error(t('checkout.upload_proof')); return; }
    ...
    try {
      ...
      const res = await checkoutOrder({
        address: fullAddress,
        phone_number: phone,
        payment_method: isWallet ? 'wallet' : 'cash_on_delivery',
    ...
    <div className="bg-gray-50 dark:bg-gray-900 min-h-screen pb-10 animate-fadein">
      <div className="max-w-3xl mx-auto px-2 sm:px-4 py-6">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-card border border-gray-100 dark:border-gray-700 p-5 mb-4">
          ...
          {addresses.length === 0 ? (
            <div className="text-center py-6">
              ...
              <Link to="/addresses" className="btn-primary inline-flex items-center gap-2 text-sm"><Plus size={16} /> {t('checkout.add_address')}</Link>
            </div>
          ) : (
            <div className="space-y-2">
              {addresses.map((addr) => (
                <button key={addr.id} onClick={() => toggleAddress(addr.id)}
                  className={`w-full text-right p-4 rounded-xl border-2 transition-all ${selectedAddressId === addr.id ? 'border-[rgb(0,166,62)] bg-green-50 dark:bg-green-900/30' : 'border-gray-100 dark:border-gray-600 hover:border-gray-200 dark:hover:border-gray-500'}`}>
              ...
              {addresses.length > 0 && <Link to="/addresses" className="flex items-center gap-2 text-sm text-[rgb(0,166,62)] hover:text-[rgb(0,145,55)] font-semibold mt-2"><Plus size={16} /> {t('checkout.add_address')}</Link>}
            </div>
          )}
        </div>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-card border border-gray-100 dark:border-gray-700 p-5 mb-4">
          ...
          <input type="tel" dir="ltr" className="w-full px-4 py-2.5 border border-gray-200 dark:border-gray-600 rounded-xl focus:ring-2 focus:ring-[rgba(0,166,62,0.3)] focus:border-[rgb(0,166,62)] outline-none transition-all duration-200 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100"
            value={phone} onChange={(e) => setPhone(e.target.value)} placeholder={t('checkout.phone_placeholder')} />
        </div>
          ...
          {appliedCoupon ? (
            <div className="flex items-center justify-between p-3 bg-green-50 dark:bg-green-900/30 rounded-xl border border-green-200 dark:border-green-700">
              <div className="flex items-center gap-2">
                <Check className="text-[rgb(0,166,62)]" size={18} />
              ...
              <button onClick={handleRemoveCoupon} className="p-1 text-gray-400 hover:text-red-500"><X size={18} /></button>
            </div>
          ) : (
            <div className="flex gap-2">
              <input type="text" className="flex-1 px-4 py-2.5 border border-gray-200 dark:border-gray-600 rounded-xl focus:ring-2 focus:ring-[rgba(0,166,62,0.3)] focus:border-[rgb(0,166,62)] outline-none transition-all duration-200 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100"
                value={couponCode} onChange={(e) => { setCouponCode(e.target.value); setCouponError(''); }} placeholder={t('checkout.enter_coupon')} />
              <button onClick={handleApplyCoupon} disabled={!couponCode.trim() || validatingCoupon}
                className="bg-[rgb(0,166,62)] hover:bg-[rgb(0,145,55)] text-white font-bold py-2.5 px-5 rounded-xl transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed active:scale-95 shadow-[0_3px_10px_rgba(0,166,80,0.2)] shrink-0">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-card border border-gray-100 dark:border-gray-700 p-5 mb-4">
          ...
          <div className="space-y-3">
            {paymentMethods.filter(pm => pm.type === 'cod').map(pm => (
              <label key={pm.id} className={`flex items-center gap-3 p-4 rounded-xl border-2 cursor-pointer transition-all ${paymentMethod === 'cash_on_delivery' ? 'border-[rgb(0,166,62)] bg-green-50 dark:bg-green-900/30' : 'border-gray-100 dark:border-gray-600 hover:border-gray-200 dark:hover:border-gray-500'}`}>
                <input type="radio" name="payment" value="cash_on_delivery" checked={paymentMethod === 'cash_on_delivery'}
                  onChange={() => { setPaymentMethod('cash_on_delivery'); setProcessNumber(''); setProofImage(null); setProofPreview(null); }} className="accent-[rgb(0,166,62)] w-4 h-4" />
                <div>
                  <span className="font-semibold text-gray-700 dark:text-gray-200">{lang === 'ar' ? pm.name_ar : pm.name_en}</span>
                  <p className="text-xs text-gray-400 dark:text-gray-500">{t('checkout.cash_desc')}</p>
                </div>
              </label>
            ))}
            {paymentMethods.filter(pm => pm.type === 'wallet').map(pm => (
              <label key={pm.id} className={`flex items-center gap-3 p-4 rounded-xl border-2 cursor-pointer transition-all ${paymentMethod === `wallet_${pm.id}` ? 'border-[rgb(0,166,62)] bg-green-50 dark:bg-green-900/30' : 'border-gray-100 dark:border-gray-600 hover:border-gray-200 dark:hover:border-gray-500'}`}>
                <input type="radio" name="payment" value={`wallet_${pm.id}`} checked={paymentMethod === `wallet_${pm.id}`}
                  onChange={() => { setPaymentMethod(`wallet_${pm.id}`); setProcessNumber(''); setProofImage(null); setProofPreview(null); }} className="accent-[rgb(0,166,62)] w-4 h-4" />
                <div>
                  <span className="font-semibold text-gray-700 dark:text-gray-200">{lang === 'ar' ? pm.name_ar : pm.name_en}</span>
                  {pm.receiver_number && <p className="text-xs text-gray-500 dark:text-gray-400" dir="ltr">{pm.receiver_number}</p>}
                  {pm.receiver_name && <p className="text-xs text-gray-500 dark:text-gray-400">{pm.receiver_name}</p>}
                </div>
              </label>
            ))}
          </div>
          {paymentMethod.startsWith('wallet_') && (
            <div className="mt-4 space-y-4 p-4 bg-gray-50 dark:bg-gray-700 rounded-xl">
              <div>
                ...
                <input type="text" className="w-full px-4 py-2.5 border border-gray-200 dark:border-gray-600 rounded-xl focus:ring-2 focus:ring-[rgba(0,166,62,0.3)] focus:border-[rgb(0,166,62)] outline-none transition-all duration-200 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100"
                  value={processNumber} onChange={(e) => setProcessNumber(e.target.value)} placeholder={t('checkout.process_number_placeholder')} required />
              </div>
              <div>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-card border border-gray-100 dark:border-gray-700 p-5 mb-4">
          ...
          <div className="space-y-3">
            {(cartItems || []).map((item) => {
              ...
              const imgSrc = p?.product_image ? (p.product_image.startsWith('http') ? p.product_image : `http://localhost:5000/uploads/${p.product_image.replace(/^\/+/, '')}`) : null;
              return (
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-card border border-gray-100 dark:border-gray-700 p-5">
          <div className="space-y-2 text-sm mb-4">
            ...
            {discount > 0 && <div className="flex justify-between"><span className="text-green-600 dark:text-green-400">{t('checkout.discount')}</span><span className="font-semibold text-green-600 dark:text-green-400">-{discount.toFixed(2)} {t('cart.currency')}</span></div>}
          ...
          <button onClick={handlePlaceOrder} disabled={!selectedAddressId || (cartItems || []).length === 0 || pendingAction}
            className="w-full bg-[rgb(0,166,62)] hover:bg-[rgb(0,145,55)] text-white font-bold py-3 px-5 rounded-xl transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed active:scale-95 shadow-[0_3px_10px_rgba(0,166,80,0.2)]">
