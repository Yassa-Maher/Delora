...
export default function Addresses() {
  ...
  const openEdit = (addr) => {
    setEditing(addr.id);
    setForm({
      title: addr.title || '', city: addr.city || '', area: addr.area || '',
      street_details: addr.street_details || '', building_number: addr.building_number || '',
      floor_number: addr.floor_number || '', is_default: addr.is_default || false,
    });
    setShowForm(true);
  };
  ...
  const handleSubmit = async (e) => {
    ...
    setSaving(true);
    try {
      if (editing) { await updateAddress(editing, form); toast.success(t('addresses.updated')); }
      else { await createAddress(form); toast.success(t('addresses.added')); }
      setShowForm(false); setEditing(null); fetchAddresses();
    } catch {} finally { setSaving(false); }
  };

  const handleDelete = async (id) => {
    if (!confirm(t('addresses.confirm_delete'))) return;
    try { await deleteAddress(id); toast.success(t('addresses.deleted')); fetchAddresses(); } catch {}
  };
          ...
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-card border border-gray-100 dark:border-gray-700 p-6 mb-6">
            ...
            <form onSubmit={handleSubmit} className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div className="sm:col-span-2 flex gap-3">
                <button type="submit" disabled={saving} className="btn-primary">{saving ? t('profile.saving') : editing ? t('common.edit') : t('common.add')}</button>
              ...
              <div key={addr.id} className="bg-white dark:bg-gray-800 rounded-2xl shadow-card border border-gray-100 dark:border-gray-700 p-5">
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    ...
                    <p className="text-gray-600 dark:text-gray-300 text-sm">{addr.city}{addr.area ? `, ${addr.area}` : ''}{addr.street_details ? ` - ${addr.street_details}` : ''}</p>
                  ...
                  <div className="flex gap-2 mr-4">
                    <button onClick={() => openEdit(addr)} className="p-2 text-gray-400 hover:text-[rgb(0,166,62)] hover:bg-green-50 dark:hover:bg-green-900/30 rounded-xl transition-colors"><Edit2 size={16} /></button>
                    <button onClick={() => handleDelete(addr.id)} className="p-2 text-gray-400 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/30 rounded-xl transition-colors"><Trash2 size={16} /></button>
                  </div>
                </div>
              </div>
            ))}
