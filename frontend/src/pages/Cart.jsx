...
import { updateCartItemQuantity, removeFromCart } from '../api/cart';
...
export default function Cart() {
  ...
  const deliveryFee = parseFloat(getSetting('delivery_fee', lang)) || 30;
                ...
                return (
                  <div key={item.id} className="bg-white dark:bg-gray-800 rounded-2xl shadow-card border border-gray-100 dark:border-gray-700 p-4 flex items-center gap-4">
                    ...
                    <div className="flex items-center border border-gray-200 dark:border-gray-600 rounded-xl">
                      <button onClick={() => handleQuantity(item.id, Math.max(isWeight ? 0.1 : 1, item.quantity - (isWeight ? 0.1 : 1)))} disabled={item.quantity <= (isWeight ? 0.1 : 1) || updating[item.id]}
                        className="p-2 hover:bg-gray-50 dark:hover:bg-gray-700 rounded-r-xl"><Minus size={14} /></button>
                      {isWeight ? (
                        <input type="number" step="0.1" min="0.1"
                          ...
                          onBlur={() => {
                            ...
                            if (editingQty[item.id] !== undefined && val >= 0.1 && val !== item.quantity) handleQuantity(item.id, +val.toFixed(3));
                      ...
                      <button onClick={() => handleQuantity(item.id, +(item.quantity + (isWeight ? 0.1 : 1)).toFixed(3))} disabled={updating[item.id]}
                        className="p-2 hover:bg-gray-50 dark:hover:bg-gray-700 rounded-l-xl"><Plus size={14} /></button>
                    </div>
                    ...
                    <button onClick={() => handleRemove(item)} className="p-2 text-gray-400 hover:text-red-500 transition-colors"><Trash2 size={18} /></button>
                  </div>
                );
              })}
            </div>
