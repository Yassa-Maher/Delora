...
export default function Contact() {
  ...
  const storePhone = getSetting('store_phone', lang) || '01000000000';
  const storeEmail = getSetting('store_email', lang) || 'delora.market@gmail.com';
  const storeAddress = getSetting('store_address', lang) || t('contact.country');
          ...
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-card border border-gray-100 dark:border-gray-700 p-6">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <button type="submit" disabled={loading} className="btn-primary flex items-center gap-2">
                <Send size={16} /> {loading ? t('common.loading') : t('contact.send')}
              </button>
            </form>
          </div>
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-card border border-gray-100 dark:border-gray-700 p-6">
            ...
            <div className="space-y-6">
              <div className="flex items-start gap-4">
                <div className="w-10 h-10 rounded-full bg-green-50 dark:bg-green-900/30 flex items-center justify-center shrink-0">
                  <MapPin className="text-[rgb(0,166,62)]" size={18} />
                </div>
                <div>
                  <h3 className="font-semibold text-gray-700 dark:text-gray-200 mb-1">{t('branches.address')}</h3>
                  <p className="text-gray-500 dark:text-gray-400">{storeAddress}</p>
                </div>
              </div>
              <div className="flex items-start gap-4">
                <div className="w-10 h-10 rounded-full bg-green-50 dark:bg-green-900/30 flex items-center justify-center shrink-0">
                  <Phone className="text-[rgb(0,166,62)]" size={18} />
                </div>
                <div>
                  <h3 className="font-semibold text-gray-700 dark:text-gray-200 mb-1">{t('branches.phone')}</h3>
                  <p className="text-gray-500 dark:text-gray-400" dir="ltr">{storePhone}</p>
                </div>
              </div>
              <div className="flex items-start gap-4">
                <div className="w-10 h-10 rounded-full bg-green-50 dark:bg-green-900/30 flex items-center justify-center shrink-0">
                  <Mail className="text-[rgb(0,166,62)]" size={18} />
                </div>
                <div>
                  <h3 className="font-semibold text-gray-700 dark:text-gray-200 mb-1">{t('contact.email')}</h3>
                  <p className="text-gray-500 dark:text-gray-400">{storeEmail}</p>
                </div>
              </div>
            </div>
          </div>
