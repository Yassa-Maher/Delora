...
export const createCategory = async (req, res) => {
    ...
    if (!name_ar) {
        return res.status(400).json({ message: 'الاسم مطلوب لإنشاء القسم' });
    }

    const finalSlug = slug || name_ar.replace(/\s+/g, '-').replace(/[^\w-]/g, '').toLowerCase() + '-' + Date.now();

    try {
        const [result] = await db.query(
            'INSERT INTO categories (name_ar, name_en, slug, parent_id, image_url) VALUES (?, ?, ?, ?, ?)',
            [name_ar, name_en || null, finalSlug, parent_id || null, image_url]
        );
